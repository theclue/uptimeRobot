  ...
  data <- fromJSON(
    getURL(
      paste0("https://api.uptimerobot.com/getMonitors?apiKey=",
             ...
             ifelse(is.null(types) | is.na(types), "", paste0("&types=", paste0(unique(types), collapse = "-"), sep="")),
             ifelse(is.null(statuses) | is.na(statuses), "", paste0("&statutes=", paste0(unique(statuses), collapse = "-"), sep="")),
